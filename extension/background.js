...
const BPCE_BLUEPRINT_SCRIPT = 'scripts/oracle_lumesse_blueprint.js';
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  try {
    ...
    if (state.taleos_nomura_tab_id === tabId || state.taleos_pending_nomura?.tabId === tabId) {
    ...
    chrome.scripting.executeScript({
      ...
      func: (fillId, text) => {
        try {
          ...
          console.log('[Taleos KO fill] élément trouvé — offsetH=', el.offsetHeight, 'valeur avant=', JSON.stringify(el.value.slice(0, 20)));

          // ── Étape 1 : native setter sur le textarea ciblé ─────────────────
          el.focus();
          el.setSelectionRange?.(0, el.value.length);
          const desc = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value');
          if (desc?.set) desc.set.call(el, text);
          else el.value = text;
          console.log('[Taleos KO fill] native setter appliqué, valeur=', JSON.stringify(el.value.slice(0, 40)));

          // ── Étape 2 : événements pour notifier Knockout ───────────────────
          // Knockout's textInput binding écoute 'input' sur l'élément.
          // On envoie aussi 'change' et 'keyup' pour les autres listeners Oracle.
          el.dispatchEvent(new InputEvent('input', { bubbles: true, cancelable: true, inputType: 'insertText', data: text }));
          el.dispatchEvent(new Event('change', { bubbles: true }));
          el.dispatchEvent(new KeyboardEvent('keyup', { key: 'End', bubbles: true }));
          console.log('[Taleos KO fill] événements dispatché, valeur finale=', JSON.stringify(el.value.slice(0, 40)));
        } catch (e) {
...
const CONNECTION_TEST_URLS = {
  ...
  rothschild: 'https://rothschildandco.wd3.myworkdayjobs.com/Rothschildandco_Lateral/login'
};
...
async function runTestConnection(msg) {
  ...
  async function closeConnectionTestTabs() {
    ...
    if (bankId === 'rothschild') {
      try {
        const rothTabs = await chrome.tabs.query({ windowId: testWindowId, url: ['https://rothschildandco.wd3.myworkdayjobs.com/*'] });
        for (const t of rothTabs) idsToClose.add(t.id);
      } catch (_) {}
    }
  ...
  try {
    ...
    // Morgan Stanley / Bank of America / Rothschild (Workday) : formulaire pas encore rendu par React → attendre et relancer (max 3 fois)
    if ((bankId === 'morgan_stanley' || bankId === 'bank_of_america' || bankId === 'rothschild') && fillRes?.[0]?.result?.needRetry && fillRes[0].result.phase === 'wait_form') {
    ...
    // Morgan Stanley / Bank of America / Rothschild (Workday) : si déjà connecté → sign out en 2 étapes puis retry
    if ((bankId === 'morgan_stanley' || bankId === 'bank_of_america' || bankId === 'rothschild') && fillRes?.[0]?.result?.needRetry) {
...
async function fetchProfile(uid, bankId, token) {
  ...
  return {
    ...
    bofa_relatives_details:      (profile.bofa_relatives_details        || '').trim(),
    bofa_referred:               (profile.bofa_referred                 || 'No').trim(),
    bofa_referrer_name:          (profile.bofa_referrer_name            || '').trim(),
    ...
    bofa_other_business_details: (profile.bofa_other_business_details   || '').trim(),
    bofa_medical_condition:      (profile.bofa_medical_condition        || 'No').trim(),
    bofa_medical_details:        (profile.bofa_medical_details          || '').trim(),
    bofa_additional_info:        (profile.bofa_additional_info          || 'No').trim(),
    bofa_additional_info_details:(profile.bofa_additional_info_details  || '').trim(),
    bofa_right_to_work:          (profile.bofa_right_to_work            || 'Yes').trim(),
    bofa_right_to_work_type:     (profile.bofa_right_to_work_type       || '').trim()
  };
}
